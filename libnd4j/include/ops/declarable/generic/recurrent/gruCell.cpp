...
CUSTOM_OP_IMPL(gru_cell, 5, 1, false, 0, 0) {
    ...
    NDArray<T>* hi = INPUT_VARIABLE(1);                     // previous/initial cell output [bS x numUnits],  that is at previous time step t-1
    ...
    const int numUnits = hi->sizeAt(1);

    const std::string hiShape        = ShapeUtils<T>::shapeAsString(hi);
    const std::string hiCorrectShape = ShapeUtils<T>::shapeAsString({bS, numUnits});
    ...
    REQUIRE_TRUE(hiShape == hiCorrectShape, 0, "GRU_CELL operation: wrong shape of previous cell output array, expected is %s, but got %s instead !", hiCorrectShape.c_str(), hiShape.c_str());
    ...
    helpers::gruCell({x, hi, Wx, Wh, b}, h);
...
DECLARE_SHAPE_FN(gru_cell) {    
    ...
    const Nd4jLong* hiShapeInfo = inputShape->at(1);                     // previous cell output [bS x numUnits],  that is at previous time step t-1
    ...
    const int numUnits = hiShapeInfo[2];
    ...
    REQUIRE_TRUE(hiShape == hiCorrectShape, 0, "GRUCELL operation: wrong shape of previous cell output array, expected is %s, but got %s instead !", hiCorrectShape.c_str(), hiShape.c_str());
    ...
    shape::updateStrides(hShapeInfo, shape::order(const_cast<Nd4jLong*>(hiShapeInfo)));
